    ...
    return (
        ...
        ${location.pathname === "/" && "bg-light"}`}>
            ...
            ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
                ...
                <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
                    <button className="cursor-pointer" onClick={()=>{
                        isOwner ? navigate("/owner/dashboard") : changeRole()
                    }}>{isOwner ? "Dashboard" : "List cars"}</button>
                    <button className="cursor-pointer px-8 py-2 bg-primary
                    hover:bg-primary-dull transition-all text-white rounded-lg" onClick={()=>{
            ...
            <button className="sm:hidden cursor-pointer" aria-label="Menu" onClick={()=>{
                ...
                <img src={open ? assets.close_icon : assets.menu_icon} alt="manu" />
            </button>
        </div>
    )
}
